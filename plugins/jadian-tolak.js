...
let handler = async (m, { conn, text, participants, groupMetadata }) => {
  ...
  try {
    ...
    if(user === m.sender) return conn.reply(m.chat, `Tidak bisa berpacaran dengan diri sendiri! lu stres yah bang`, m)
    if(user === conn.user.jid) return conn.reply(m.chat, `*Tidak bisa berpacaran dengan saya t_t apalagi si amelll`, m)
    ...
    if(global.db.data.users[user].pasangan != m.sender){
      ...
      conn.reply(m.chat,`Mampus sia menolak @${user.split('@')[0]} untuk menjadi pasangan anda!`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }
	}	
